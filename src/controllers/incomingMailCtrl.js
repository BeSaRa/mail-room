module.exports = function (app) {
    app.controller('incomingMailCtrl', function (incomingMailService,
                                                 $q,
                                                 _,
                                                 mailService,
                                                 helpService,
        ...
        self.controllerName = 'incomingMailCtrl';
        helpService.setHelpTo('incoming-mails');
        ...
        var _updateGridByStatusMap = function () {
            ...
            _gridsByStatus[lookupService.statusTypesKeys.expected] = self.grid.expectedMails;
        };
        ...
        self.setMailsByStatusType = function (statusType, mails, skipOverrideCopy) {
            if (statusType) {
                ...
                } else if (statusType === lookupService.statusTypesKeys.expected) {
                    self.expectedMails = mailService.categorizeMailsByStatusType(mails, statusType);
                    if (!skipOverrideCopy)
                        self.expectedMailsCopy = generator.shallowCopyArray(self.expectedMails);
                }
            }
        };
        ...
        self.tabsToShow = {
            expectedMails: {
                name: 'expectedMails',
                key: 'lbl_incoming_expected_mails'
            },
            newMails: {
                name: 'newMails',
                key: 'lbl_incoming_new_mails'
            },
            sentMails: {
                name: 'sentMails',
                key: 'lbl_incoming_sent_mails'
            },
            receivedMails: {
                name: 'receivedMails',
                key: 'lbl_incoming_received_mails'
            }
        };
        self.showTab = function (tab) {
            return !tab.hide;
        };
        self.selectedTabName = self.tabsToShow.expectedMails.name;
        ...
        self.openAddMailDialog = function ($event) {
            incomingMailService.controllerMethod.addDialog(self.grid.receivedMails, $event);
        };
        ...
        self.viewMail = function (mail, grid, $event) {
            incomingMailService.controllerMethod.editDialog(mail, grid, true, $event);
        };
        ...
        self.openEditMailDialog = function (mail, grid, $event) {
            incomingMailService.controllerMethod.editDialog(mail, grid, false, $event);
        };
        ...
        /**
         * @description Show linked mail
         * @param mail
         * @param grid
         * @param $event
         */
        self.showLinkedMail = function (mail, grid, $event) {
            incomingMailService.controllerMethod.showLinkedMail(mail, $event);
        };
        ...
        self.grid = {
            ...
            expectedMails: {
                ...
                limitOptions: gridService.getGridLimitOptions(self.expectedMails),
                columns: {
                    referenceNo: {
                    ...
                    postType: {
                        ...
                        sortKey: function () {
                            if (gridService.isGridServerScope(self.grid.expectedMails.sortScope)) {
                    ...
                    senderEntity: {
                        ...
                        sortKey: function () {
                            if (gridService.isGridServerScope(self.grid.expectedMails.sortScope)) {
                                ...
                                return 'senderEntity' + 'Embedded.entityName' + (langService.current === 'en' ? 'En' : '');
                            }
                            return self.getSortingKey('senderEntity', 'Entity');
                        }
                    },
                    ...
                    gridActions: {
                        ...
                        hide: function () {
                            return !self.expectedMails.length;
                        }
                    }
                },
                hide: true,
                columnsCount: function (includeHidden) {
                    return gridService.getColumnsCount(self.grid.expectedMails, includeHidden);
                },
                colSpan: function () {
                    return gridService.getColSpan(self.grid.expectedMails);
                },
                showColumn: function (column, isHeader) {
                    return gridService.isShowColumn(self.grid.expectedMails, column, isHeader);
                },
                searchText: '',
                searchCallback: function () {
                    var searchedRecords = mailService.searchMailGridData(self.grid.expectedMails, self.expectedMailsCopy, incomingMailService.totalRecords);
                    self.setMailsByStatusType(lookupService.statusTypesKeys.expected, searchedRecords, true);
                },
                sortScope: gridService.gridScope.client,
                sortCallback: function (sortKey) {
                    if (gridService.isGridServerScope(self.grid.expectedMails.sortScope)) {
                        ...
                        if (self.grid.expectedMails.filterGridCriteria.criteria) {
                            var defer = $q.defer();
                            self.grid.expectedMails.progress = defer.promise;
                            incomingMailService._filterCallback(self.grid.expectedMails, lookupService.statusTypesKeys.expected)
                                .then(function (result) {
                                    self.grid.expectedMails.selectedRecords = [];
                                    // result will contain only records with this status but to assign to variable, call setMailsByStatusType method
                                    self.setMailsByStatusType(lookupService.statusTypesKeys.expected, result);
                                    self.grid.expectedMails.searchCallback();
                                    defer.resolve(true);
                                });
                        } else {
                            self.grid.expectedMails.reload(self.grid.expectedMails.page)
                                .then(function () {
                                    self.grid.expectedMails.searchCallback();
                                });
                        }
                    } else {
                        self.expectedMails = gridService.sortGridData(self.grid.expectedMails, self.expectedMails);
                    }
                },
                filterGridCriteria: {reset: false, showReset: false, filterScope: gridService.gridScope.client},
                filterGridCallback: function ($event) {
                    var defer = $q.defer();
                    incomingMailService.controllerMethod.filterGridDialog(lookupService.statusTypesKeys.expected, self.grid.expectedMails, $event)
                        .then(function (result) {
                            counterService.loadCounters();
                            self.grid.expectedMails.selectedRecords = [];
                            // result will contain only records with this status but to assign to variable, call setMailsByStatusType method
                            self.setMailsByStatusType(lookupService.statusTypesKeys.expected, result);
                            gridService.resetSearchText(self.grid.expectedMails);
                            self.grid.expectedMails.progress = defer.promise;
                            defer.resolve(true);
                ...
                printFilterCallback: function ($event) {
                    printService.printSearchGrid(self.expectedMailsCopy, self.grid.expectedMails, $event);
                },
                pagingScope: gridService.gridScope.client,
                pagingCallback: function (pageNumber, limit) {
                    if (gridService.isGridServerScope(self.grid.expectedMails.pagingScope) && self.isTabSelected(self.tabsToShow.expectedMails)) {
                        ...
                        if (self.grid.expectedMails.filterGridCriteria.criteria) {
                            ...
                            self.grid.expectedMails.reload(pageNumber);
                        }
                    } else {
                        // do nothing
                    }
                ...
                reload: function (pageNumber, $event) {
                    ...
                    if (self.isHardReload) {
                        gridService.resetSorting(self.grid.expectedMails, '-registerDate');
                        gridService.resetSearchText(self.grid.expectedMails);
                    }
                    if (pageNumber)
                        self.grid.expectedMails.page = pageNumber;

                    if (self.grid.expectedMails.filterGridCriteria.criteria) {
                        incomingMailService._filterCallback(self.grid.expectedMails, lookupService.statusTypesKeys.expected, $event)
                            .then(function (result) {
                        ...
                        incomingMailService.loadMails(lookupService.statusTypesKeys.expected, self.grid.expectedMails, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
                ...
                actions: [
                    // Mail Information Text
                    {
                        ...
                        hide: true,
                        grid: function () {
                            return self.grid.expectedMails;
                        },
                        class: "action-green",
                    ...
                    // Show Linked Mail
                    {
                        ...
                        text: 'grid_action_show_linked_mail',
                        shortcut: true,
                        callback: self.showLinkedMail,
                        grid: function () {
                            return self.grid.expectedMails;
                        },
                        class: "action-green",
                        checkShow: function (action, model) {
                            return gridService.checkToShowAction(action, model) && !!model.getRelatedMailId();
                        },
                        hide: true
                    }
                ],
            ...
            newMails: {
                ...
                searchText: '',
                searchCallback: function () {
                    var searchedRecords = mailService.searchMailGridData(self.grid.newMails, self.newMailsCopy, incomingMailService.totalRecords);
                ...
                sortCallback: function (sortKey) {
                    if (gridService.isGridServerScope(self.grid.newMails.sortScope)) {
                        ...
                        if (self.grid.newMails.filterGridCriteria.criteria) {
                            ...
                            incomingMailService._filterCallback(self.grid.newMails, lookupService.statusTypesKeys.new)
                                .then(function (result) {
                ...
                filterGridCallback: function ($event) {
                    var defer = $q.defer();
                    incomingMailService.controllerMethod.filterGridDialog(lookupService.statusTypesKeys.new, self.grid.newMails, $event)
                        .then(function (result) {
                            counterService.loadCounters();
                            self.grid.newMails.selectedRecords = [];
                            // result will contain only records with this status but to assign to variable, call setMailsByStatusType method
                ...
                reload: function (pageNumber, $event) {
                    ...
                    if (self.grid.newMails.filterGridCriteria.criteria) {
                        incomingMailService._filterCallback(self.grid.newMails, lookupService.statusTypesKeys.new, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
                        ...
                        incomingMailService.loadMails(lookupService.statusTypesKeys.new, self.grid.newMails, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
                ...
                actions: [
                    ...
                    // Receive
                    {
                        ...
                        params: lookupService.statusTypesKeys.mobile_received,
                        class: "action-green",
                        checkShow: gridService.checkToShowAction
                    },
                    // Show Barcode
            ...
            sentMails: {
                ...
                sortCallback: function (sortKey) {
                    if (gridService.isGridServerScope(self.grid.sentMails.sortScope)) {
                        ...
                        if (self.grid.sentMails.filterGridCriteria.criteria) {
                            ...
                            incomingMailService._filterCallback(self.grid.sentMails, lookupService.statusTypesKeys.sent)
                                .then(function (result) {
                ...
                searchText: '',
                searchCallback: function () {
                    var searchedRecords = mailService.searchMailGridData(self.grid.sentMails, self.sentMailsCopy, incomingMailService.totalRecords);
                ...
                filterGridCallback: function ($event) {
                    ...
                    incomingMailService.controllerMethod.filterGridDialog(lookupService.statusTypesKeys.sent, self.grid.sentMails, $event)
                        .then(function (result) {
                            counterService.loadCounters();
                            self.grid.sentMails.selectedRecords = [];
                            // result will contain only records with this status but to assign to variable, call setMailsByStatusType method
                ...
                reload: function (pageNumber, $event) {
                    ...
                    if (self.grid.sentMails.filterGridCriteria.criteria) {
                        incomingMailService._filterCallback(self.grid.sentMails, lookupService.statusTypesKeys.sent, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
                        ...
                        incomingMailService.loadMails(lookupService.statusTypesKeys.sent, self.grid.sentMails, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
            ...
            receivedMails: {
                ...
                columns: {
                    ...
                    entryType: {
                        ...
                        sortKey: function () {
                            if (gridService.isGridServerScope(self.grid.receivedMails.sortScope)) {
                                return 'entryTypeEmbedded.' + (langService.current + 'Name');
                            }
                            return self.getSortingKey('entryTypeLookup', 'Lookup');
                        },
                        hide: true
                    ...
                    priority: {
                        ...
                        sortKey: function () {
                            if (gridService.isGridServerScope(self.grid.receivedMails.sortScope)) {
                                return 'priorityTypeEmbedded.' + (langService.current + 'Name');
                            }
                            return self.getSortingKey('priorityTypeLookup', 'Lookup');
                        },
                        hide: true
                ...
                sortCallback: function (sortKey) {
                    if (gridService.isGridServerScope(self.grid.receivedMails.sortScope)) {
                        ...
                        if (self.grid.receivedMails.filterGridCriteria.criteria) {
                            ...
                            incomingMailService._filterCallback(self.grid.receivedMails, lookupService.statusTypesKeys.received)
                                .then(function (result) {
                ...
                searchCallback: function () {
                    ...
                    var searchedRecords = mailService.searchMailGridData(self.grid.receivedMails, self.receivedMailsCopy, incomingMailService.totalRecords);
                ...
                filterGridCallback: function ($event) {
                    ...
                    incomingMailService.controllerMethod.filterGridDialog(lookupService.statusTypesKeys.received, self.grid.receivedMails, $event)
                        .then(function (result) {
                            counterService.loadCounters();
                            self.grid.receivedMails.selectedRecords = [];
                            // result will contain only records with this status but to assign to variable, call setMailsByStatusType method
                ...
                reload: function (pageNumber, $event) {
                    ...
                    if (self.grid.receivedMails.filterGridCriteria.criteria) {
                        incomingMailService._filterCallback(self.grid.receivedMails, lookupService.statusTypesKeys.received, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
                        ...
                        incomingMailService.loadMails(lookupService.statusTypesKeys.received, self.grid.receivedMails, $event)
                            .then(function (result) {
                                loadDefer.resolve(result);
                            })
                            .catch(function (error) {
        ...
        self.reloadGrid = function ($event) {
            ...
            } else if (self.isTabSelected(self.tabsToShow.expectedMails)) {
                self.grid.expectedMails.reload(self.grid.expectedMails.page, $event);
            }
        };
    });
};
