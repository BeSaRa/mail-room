module.exports = function (app) {
    app.service('loginDialogService', function (dialog, $q, langService, mailRoomTemplate, $rootScope) {
        'ngInject';
        var self = this;
        // service name
        ...
        self.openLoginDialog = function (url) {
            ...
            return self.promise = dialog.showDialog({
                template: mailRoomTemplate.getPopup('login'),
        ...
        self.displaySessionMessage = function () {
            if (!self.sessionMessage) {
                dialog.errorMessage(langService.get('auth_session_expired')).then(function () {
                    self.sessionMessage = false;
                });
